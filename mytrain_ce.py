...
class Args:
    def __init__(self):
        ...
        self.load_checkpoint_dir = f"/home/gpuadmin/Desktop/RWKV/checkpoints/sum_spot3_ratio50"
        self.loadormodel = "model"
        self.save_checkpoint_dir = f"/home/gpuadmin/Desktop/RWKV/checkpoints/{self.name}"
        self.max_spots = 3
        self.spot_size = 0.2
        self.spot_teacher_forcing = 3
...
def custom_collate_fn(batch):
    ...
    batch["locs"] = [torch.tensor(x).to(args.dtype) for x in batch["locs"]]
...
def move_to_device(batch, device):
    for key, value in batch.items():
        if key!="real_images" and key!="locs":
...
def calculate_iou(pred_boxes, label_boxes):
    ...
    label_y2 = label_boxes[:, 1] + label_boxes[:, 3]
...
def calculate_accuracy(logits, labels):
    ...
    with torch.no_grad():
        ...
        return accuracy
import math
def calculate_ciou(pred_boxes: torch.Tensor,
                   label_boxes: torch.Tensor,
                   eps: float = 1e-7) -> torch.Tensor:
    """
    pred_boxes, label_boxes: (N, 4) 텐서
      - 각 행은 [cx, cy, w, h] 형태라고 가정
    eps: 계산 상의 안정성을 위한 작은 값

    return: 스칼라(배치 평균 CIoU Loss)
    """

    # ----- 1) [cx, cy, w, h] -> [x1, y1, x2, y2] 변환 -----
    # 예측 박스
    ...
    # 라벨 박스(정답 박스)
    label_cx, label_cy, label_w, label_h = label_boxes[:, 0], label_boxes[:, 1], label_boxes[:, 2], label_boxes[:, 3]
    label_x1 = label_cx - (label_w / 2)
    label_y1 = label_cy - (label_h / 2)
    label_x2 = label_cx + (label_w / 2)
    label_y2 = label_cy + (label_h / 2)
    ...
    # ----- 3) DIoU를 위한 추가 항목(중심 거리 / 가장 작은 외접 박스 대각선 길이)
    # 중심 거리
    center_dist = (pred_cx - label_cx)**2 + (pred_cy - label_cy)**2

    # 가장 작은 외접 박스의 x1, y1, x2, y2
    enc_x1 = torch.min(pred_x1, label_x1)
    enc_y1 = torch.min(pred_y1, label_y1)
    enc_x2 = torch.max(pred_x2, label_x2)
    enc_y2 = torch.max(pred_y2, label_y2)

    # 외접 박스 대각선 길이 (제곱)
    c = (enc_x2 - enc_x1)**2 + (enc_y2 - enc_y1)**2 + eps

    # DIoU 항
    rDIoU = center_dist / c

    # ----- 4) CIoU를 위한 추가 항목(aspect ratio 차이 v, alpha)
    # v: 너비-높이 비율(Aspect Ratio) 차이
    # arctan(w/h) 사용
    v = (4 / (math.pi ** 2)) * torch.pow(
        torch.atan(label_w / (label_h + eps)) - torch.atan(pred_w / (pred_h + eps)),
        2
    )

    # alpha: v와 IoU값에 따라 동적으로 결정
    with torch.no_grad():
        S = 1 - iou  # (1 - IoU)
        alpha = v / (S + v + eps)

    # ----- 5) CIoU Loss -----
    # CIoU = IoU - rDIoU - alpha*v
    # Loss = 1 - IoU + rDIoU + alpha*v
    ciou_loss = (1 - iou) + rDIoU + alpha * v
...
def validate(model_engine, val_loader, device):
    ...
    with torch.no_grad():
        ...
        for step, batch in progressbar:
            ...
            ciou_loss = torch.tensor([0]).to(args.dtype)
            count = 0
            for i in range(batch["locs"]):
                til = min(batch["locs"][i].shape[0],locs.shape[0])
                ciou_loss += calculate_ciou(locs[:til,i,:].to(torch.float32),batch["locs"][i][:til].to(torch.float32)).to(args.dtype)
                count += 1
            ciou_loss /= count
            ...
            total_loss += loss.item()*args.answer_loss + ciou_loss.item()*args.spot_loss
            total_accuracy += accuracy
            total_iou += iou
            total_ciou_loss += ciou_loss.item()
            total_ans_loss += loss.item()

    avg_loss = total_loss / len(val_loader)
    avg_iou = total_iou / len(val_loader)
    avg_accuracy = total_accuracy / len(val_loader)
    avg_ans_loss = total_ans_loss / len(val_loader)
    avg_ciou_loss = total_ciou_loss / len(val_loader)
    return avg_loss, avg_accuracy, avg_iou, avg_ans_loss, avg_ciou_loss
...
for epoch in range(args.epoch_begin, args.max_epochs):
    ...
    for step, batch in progress_bar:
        ...
        loss = loss.view(shift_labels.size()).sum(1)/valid_lengths
        loss = loss.mean()
        tot_loss = loss*args.answer_loss + ciou_loss*args.spot_loss
        # Accuracy 계산
        accuracy = calculate_accuracy(shift_logits, shift_labels)
        iou = calculate_iou(locs[0].to(torch.float32), batch["locs"].to(torch.float32))
        ...
        train_iou += iou
        train_ans_loss += loss.item()
        train_ciou_loss += ciou_loss.item()

        time_logs["loss_accuracy"].append(time.time()-start_time);start_time=time.time()

        model_engine.backward(tot_loss)
        model_engine.step()
    ...
    train_ans_loss = train_ans_loss / len(train_loader)
    train_ciou_loss = train_ciou_loss / len(train_loader)

    # Validation
    val_loss, val_accuracy, val_iou, val_ans_loss, val_ciou_loss = validate(model_engine, val_loader, device)
    deepspeed.comm.barrier(device_ids=[0,1,2,3])
    ...
    if torch.distributed.is_initialized():
        ...
        train_ans_loss_tensor = torch.tensor(train_ans_loss).to(device)
        val_ans_loss_tensor = torch.tensor(val_ans_loss).to(device)
        train_ciou_loss_tensor = torch.tensor(train_ciou_loss).to(device)
        val_ciou_loss_tensor = torch.tensor(val_ciou_loss).to(device)
        ...
        torch.distributed.all_reduce(train_ans_loss_tensor)
        torch.distributed.all_reduce(val_ans_loss_tensor)
        torch.distributed.all_reduce(train_ciou_loss_tensor)
        torch.distributed.all_reduce(val_ciou_loss_tensor)
        ...
        train_ans_loss = train_ans_loss_tensor.item() / torch.distributed.get_world_size()
        val_ans_loss = val_ans_loss_tensor.item() / torch.distributed.get_world_size()
        train_ciou_loss = train_ciou_loss_tensor.item() / torch.distributed.get_world_size()
        val_ciou_loss = val_ciou_loss_tensor.item() / torch.distributed.get_world_size()
    ...
    if deepspeed.comm.get_rank() == 0:
        ...
        history["train_ans_loss"].append(train_ans_loss)
        history["val_ans_loss"].append(val_ans_loss)
        history["train_ciou_loss"].append(train_ciou_loss)
        history["val_ciou_loss"].append(val_ciou_loss)
        with open(history_path, 'w') as f:
            json.dump(history, f, indent=4)
        print(f"Epoch [{epoch+1}/{args.max_epochs}] completed.")
        print(f" Train Loss: {train_loss:.4f} | Train Accuracy: {train_accuracy:.4f} | Train Iou: {train_iou:.4f} | Train Ans Loss: {train_ans_loss:.4f} | Train CIoU Loss: {train_ciou_loss:.4f}")
        print(f" Valid Loss: {val_loss:.4f} | Valid Accuracy: {val_accuracy:.4f} | Valid Iou: {val_iou:.4f} | Valid Ans Loss: {val_ans_loss:.4f} | Valid CIoU Loss: {val_ciou_loss:.4f}")
