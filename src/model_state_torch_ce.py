...
class RWKV_II(pl.LightningModule):
    def __init__(self, args):
        ...
        self.emb_spot = nn.Linear(args.vocab_size, 4)
    ...
    def zero_pad_to_square(self,tensor):
        """
        주어진 텐서를 정사각형 형태로 제로 패딩합니다.

        Args:
            tensor (torch.Tensor): 패딩할 텐서. 형태는 (C, H, W) 또는 (H, W).

        Returns:
            torch.Tensor: 정사각형으로 패딩된 텐서.
        """
        # 텐서의 차원 확인
        if tensor.dim() == 3:
            C, H, W = tensor.shape
            ...
            raise ValueError("텐서는 2차원 또는 3차원이어야 합니다.")

        # 높이와 너비 비교
        if H == W:
            return tensor  # 이미 정사각형인 경우 그대로 반환
        ...
        if H > W:
            ...
            padding = (pad_left, pad_right, 0, 0)  # (왼쪽, 오른쪽, 위, 아래)
        else:
            pad_total = W - H
            pad_top = pad_total // 2
            pad_bottom = pad_total - pad_top
            padding = (0, 0, pad_top, pad_bottom)  # (왼쪽, 오른쪽, 위, 아래)

        # 패딩 적용
        if C is not None:
            # (C, H, W) 형태인 경우, F.pad의 padding 순서는 (왼쪽, 오른쪽, 위, 아래)
            padded_tensor = F.pad(tensor, padding, "constant", 0)
        else:
            # (H, W) 형태인 경우, 차원을 맞춰서 패딩 적용
            padded_tensor = F.pad(tensor.unsqueeze(0), padding, "constant", 0).squeeze(0)

        return padded_tensor
    def extract_spot(self, images, loc, size, fake_loc, mask):
        """
        ...
        """
        spots = []
        locs = torch.zeros((len(images),4)).to(loc.dtype).to(loc.device)
        for b, image in enumerate(images):
            ...
            if mask[b]==0:
                ...
                x = (fake_loc[b, 0] * W).long()
                y = (fake_loc[b, 1] * H).long()
            ...
            start_y = y
            if mask[b]==0:
                end_x = x + max(((loc[b, 2])*W).long(),1)
                end_y = y + max(((loc[b, 3])*H).long(),1)
            else:
                end_x = x + max(((fake_loc[b, 2])*W).long(),1)
                end_y = y + max(((fake_loc[b, 3])*H).long(),1)
    ...
    def forward_without_last_image(self, input_ids, images, real_images, max_spots, do_sample, temperature, top_p, max_new_tokens, stop_token_idx, spots_tf) -> list[int]:
        ''' one mode to generate, only generate one sample at a time
        # input_ids: [1, seq_len]
        # images: [1, 1, 3, 224, 224]
        # do_sample: bool
        ...
        for i in range(max_spots):
            if i==0:
                ...
                loc = torch.sigmoid(self.emb_spot(logits))
                fake_loc = torch.zeros_like(loc)
                mask = torch.zeros_like(loc.shape[0])
                for j in range(loc.shape[0]):
                    if min(spots_tf[j].shape[0],self.args.spot_teacher_forcing)>=i:
                        fake_loc[j] = spots_tf[j][i-1]
                        mask[j] = 1

                spot, loc = self.extract_spot(real_images, loc, 336, fake_loc, mask)
                locs.append(loc)
        ...
        mask = torch.zeros_like(loc.shape[0])
        for j in range(loc.shape[0]):
            if min(spots_tf[j].shape[0],self.args.spot_teacher_forcing)>=max_spots:
                fake_loc[j] = spots_tf[j][i-1]
                mask[j] = 1
        spot, loc = self.extract_spot(real_images, loc, 336, fake_loc, mask)
        locs.append(loc)
